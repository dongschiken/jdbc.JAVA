...
-- emp 테이블이 어떤 컬럼으로 이루어져 있는지 확인

-- DESC == DESCRIBE (DESC가 DESCRIBE의 줄임말)
-- SCOTT.emp == emp (어차피 계정이 scott이라서)
DESC SCOTT.emp;
DESCRIBE emp;

EMPNO    NOT NULL NUMBER(4) // null 허용x(필수로 입력해야한다.) NUMBER(4) 4자리 정수
ENAME             VARCHAR2(10) // 문자열(10바이트)
JOB               VARCHAR2(9)  // 문자열(9바이트)
MGR               NUMBER(4)    // 정수 (4자리)
HIREDATE          DATE         // 날짜
SAL               NUMBER(7,2)  // 실수 - 소수점 2자리 
COMM              NUMBER(7,2)  // 실수 - 소수점 2자리
DEPTNO            NUMBER(2)     
...
-- DISTINCT : 출력할 컬럼이 중복될때 한번만 출력
SELECT DISTINCT job
FROM emp;

-- 사원번호 넘버 ~ 이름 ~ 입사날짜 전부 같으면 한번만 출력(중복제거
...
-- 도구 - 환경설정 - NLS(National Language Support) : 날짜형식 RR/MM/DD
-- NLS : 국가 지원 언어 
-- NLS의 파라미터는 SESSION > CLIENT > SERVER 이렇게 분류된다. 
-- server, client, session의 환경이 서로 다르다면, session에서 설정한 환경을 따라 가게 된다. 예를 들어, server의 통화기호가 $, client는 ?, session은 €라면 통화기호는 €를 따르게 된다.
-- EX_) 오라클 서버                  클라이언트 도구 출장
--       미국(달러)                       일본(엔화)
--                  통화단위를 "원"이 나오게 하고싶다. (Session에서 "원"으로 설정되어 있으면 된다.)
...
-- insa 테이블 생성 + INSERT문을 통해 60명의 사원을 추가하는 작업 -- 


-- emp 테이블에서 사원 정보 조회(사원 번호, 사원명, 입사일자, 월급(pay) == 기본급(sal) + 커미션(comm))

SELECT empno, ename, hiredate, 
        sal, 
--        NVL2(comm, comm, 0) COMM, 
        COALESCE (comm, comm, 0) COMM,
        COALESCE ( sal + comm, sal, 0) PAY
--        sal + NVL(comm, 0) pay
        FROM emp;
-- 오라클에서 NULL 의미 ==> 미확인된 값(모르는 값, 정의되지 않은 값)
-- null을 다른 값(0)으로 대체해서 처리하자
-- null을 처리하는 오라클 함수 : NVL(), NVL2(), NULLIF(), COALESCE() 4가지
-- NVL(expr1,expr2) // 만약 expr1이 널이라면 expr2로 바꾼다.
...
-- 문제 emp 테이블에서 mgr(직속상사 사원번호) 컬럼이 null인 사원
SELECT *
FROM emp ;


-- 숫자를 문자로 바꿀 때 => TO_CHAR(바꿀 숫자);
-- 문자를 숫자로 바꿀 때 => TO_NUMBER(바꿀 문자);
--SELECT NVL(mgr , 11111) mgr
SELECT NVL(TO_CHAR(mgr) , 'ceo') mgr
SELECT NVL2( mgr, ' '||mgr, 'ceo')
FROM emp E;
--WHERE mgr is null;
...
FROM emp
WHERE deptno = 10;
-- 부서번호가 10번인 사원들의 정보만 출력(조회)

-- 문제 : 부서번호가 10번이 아닌 사원들의 정보만 출력
-- 오라클에서 부정연산자 (not)
SELECT *
FROM emp
WHERE not(deptno = 10);
...
-- [문제] emp 테이블에서 10, 20번 사원들만 조회(출력);
SELECT *
FROM emp
WHERE deptno = 10 OR deptno = 20;

-- 자바의 논리 연산자   &&  ||  !
-- 오라클의 논리 연산자 AND OR NOT

-- NULL을 비교할때는 IS NULL 또는 IS NOT NULL을 사용한다.
SELECT *
FROM emp
WHERE comm IS NULL;
WHERE comm IS NOT NULL;

-- [ 문제 ] : 사원명이 KING인 사원의 정보를 조회 
-- 검색할 때 대소문자 구분 해야한다. (다른 쿼리문은 상관없음) WHERE ename = 'KING'; 
-- COUNT() 함수는 그룹함수, 복수행 함수(여러 행이 들어가서 하나로 처리되는), 집계함수
SELECT *
FROM emp
WHERE ename LIKE UPPER('king');
WHERE ename = 'KING';

SELECT ename, LOWER(ename) lowEname, UPPER(ename) upEname, INITCAP(ename) initEname
FROM emp
WHERE job = 'CLERK';

-- [ 문제 ] : 월급(pay)이 2000이상 4000이하
-- 처리 순서때문에 SELECT 에서 만들어진 PAY가 WHERE에서 인식이 안되는 문제가 생김
-- ORA-00904: "PAY": invalid identifier :  PAY를 인식할 수 없다.
-- 로직 자체를 넘겨주면 해결 가능하다. ==> sal + NVL(comm , 0)
-- BETWEEN 에 문자를 넣으면 'B' 이상 'C' 이하로도 표현 가능
SELECT ename,
       sal + NVL(comm , 0) pay
FROM emp
